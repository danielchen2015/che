...
class User extends Base
{
    ...
    /**
     * @SWG\Get(
     *     path="/api/User/loginWxxcxNew?code={code}&encryptedData={encryptedData}&iv={iv}",
     *     tags={"1-用户管理部分接口"},
     *     operationId="loginWxxcxNew",
     *     summary="1.3-获取用户信息",
     *     description="获取用户信息(为小程序使用)。code\encryptedData\iv。",
     *     consumes={"application/json"},
     *     @SWG\Parameter(
     ...
     *         @SWG\Schema(ref="#/definitions/ApiResponse")
     *     ),
     *     security={{"petstore_auth":{"write:loginWxxcxNew", "read:loginWxxcxNew"}}}
     * )
     */
    public function loginWxxcxNew(Request $request)
    {
        $code = $request->param('code');
        $encryptedData = $request->param('encryptedData');
        $iv = $request->param('iv');
